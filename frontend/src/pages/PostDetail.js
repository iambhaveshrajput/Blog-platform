...
// Safe date formatter — never throws on null/invalid dates
...
function PostDetail() {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      fetchPost();
      showToast(
        response.data.liked ? 'Post liked!' : 'Post unliked', 
  ...
  const handleBookmark = async () => {
    ...
    try {
      ...
      fetchPost();
      showToast(
        response.data.bookmarked ? 'Post bookmarked!' : 'Bookmark removed',
        response.data.bookmarked ? 'success' : 'info'
      );
    } catch (err) {
  ...
  return (
    <Container className="post-detail-container">
      ...
      <div className="post-actions slide-up">
        ...
        <button
          className={`action-btn ${post.is_bookmarked ? 'active' : ''}`}
